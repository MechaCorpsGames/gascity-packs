...
class BrowserContractCiTests(unittest.TestCase):
    ...
    def test_isolated_certificate_creates_and_starts_a_disposable_city_through_gc_init(self) -> None:
        ...
        self.assertIn("const requestedRootName = process.env.GC_LIVE_ROOT_NAME", script)
        self.assertIn("/^dsh-gc-live-[A-Za-z0-9_-]+$/", script)
        self.assertIn("/^dsh-live-[A-Za-z0-9_-]+$/", script)
        self.assertIn("throw new Error(`isolated live root already exists", script)
    ...
    def test_live_agent_matrix_is_a_two_provider_schema_v2_pack(self) -> None:
        ...
        self.assertIn('option_defaults = { effort = "low" }', claude)
        self.assertIn('option_defaults = { effort = "low" }', codex)
