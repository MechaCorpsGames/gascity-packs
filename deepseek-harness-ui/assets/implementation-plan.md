...
Status: implemented and release-ready for the documented v1 boundary. Deterministic, stock-browser, SSH, soak, and isolated real-Claude/real-Codex gates pass for the checksum-pinned final artifact. Registry publication remains a separate release operation.
...
The release is verified by 147 plugin tests, 40 pack/install tests, TypeScript checking, production builds, ShellCheck, `gc lint`, deterministic artifact byte comparison, and checked-in headless-Chrome contract/uncertainty/soak runs against an isolated exact stock `dsh web` profile and random-port Supervisor fixture. The separate isolated live certificate passed the final artifact with real Claude and Codex sessions, authoritative nonce and tool-call/result evidence, browser rendering, follow-up submits, permanent close, and complete owned-resource cleanup.
...
- **Live provider scope and availability:** final artifact `6449a835d5d8a79ccc61e5b48d556afd3173c1f1a6b0c050abaf8a56c0973d1d` passed for Claude and Codex through the same stock-DSH/pack path on August 27, 2026. This certifies only those named providers on DSH `0.1.1-rc.2`, Chrome `151.0.7922.174`, and Gas City `1.4.0` (`a7297c511d637a3609947386f3389d76ddb2f23b`); it is not a blanket claim about every provider/version combination.
- **Fresh Claude workspace trust:** Claude Code `2.1.250` presents its workspace-trust dialog with **No, exit** selected, while Gas City `1.4.0` startup handling sends Enter under the older assumption that the safe continuation is selected. Existing trusted cities work and were certified. A brand-new untrusted Claude city may fail before the pack receives a session; fixing that dialog navigation belongs upstream in Gas City. The release fixture can safely recreate an exact previously trusted disposable path with narrowly validated `GC_LIVE_ROOT_NAME`/`GC_LIVE_CITY` values and refuses an existing target.
