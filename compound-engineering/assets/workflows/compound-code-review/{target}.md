...
validate as `gc.build.review.v1`. When the selected canonical path differs,
copy the internal report byte-for-byte to that exact adapter path. Do not copy
the gap-analysis report or review-fix summary. Verify the source and adapter
reports have identical bytes, then confirm the selected adapter report exists
and validates as `gc.build.review.v1`. Persist a derived path on the workflow
root as `gc.build.review_report_path`; preserve `gc.var.report_path` unchanged.

On repair attempts (`gc.attempt` greater than 1), read `gc.attempt_log` on the
validation-loop control bead first. Repair the complete internal report at
`gc.build.code_review_report_path`, then copy it again to the exact selected
...
- If the adapter path cannot be selected, either report cannot be validated,
  the copies differ, or required findings remain unresolved in a fix-authorized
  mode, do not approve the expansion.
